...
[![CI](https://github.com/tsegaw/pysquirrel/actions/workflows/ci.yml/badge.svg)](https://github.com/tsegaw/pysquirrel/actions/workflows/ci.yml)
[![PyPI](https://img.shields.io/pypi/v/pysquirrel.svg)](https://pypi.org/project/pysquirrel/)
[![Python 3.10+](https://img.shields.io/badge/python-3.10%2B-blue.svg)](https://www.python.org/downloads/)
[![License: MIT](https://img.shields.io/badge/license-MIT-blue.svg)](LICENSE)

Write SQL. Get typed Python. No ORM.

pysquirrel reads `.sql` files, connects to a live Postgres database, introspects your queries via `PREPARE`/`EXPLAIN`, and generates a Python module with **Pydantic v2 row models**, **async functions**, and **enum classes** — so your database layer is fully typed, validated, and IDE-friendly without writing a single model class by hand.

## What is pysquirrel?

pysquirrel is a build-time code generator for Python projects that use [asyncpg](https://github.com/MagicStack/asyncpg) to talk to PostgreSQL. You write plain SQL files; pysquirrel connects to your database, introspects each query's parameter and result types, and emits a Python module with:
...
There is no runtime dependency beyond `asyncpg` and `pydantic`. pysquirrel itself is only needed at generation time.
...
Annotations are special `--` comments that pysquirrel parses before the SQL body:
...
pysquirrel infers nullability automatically from three sources, in priority order:
...
Use positional parameters (`$1`, `$2`, ...) in your SQL. pysquirrel maps them to Python parameters named `_1`, `_2`, etc. All parameters are nullable with `= None` defaults, matching asyncpg's behavior.
...
- `name` and `org_name` are `str | None = None` because pysquirrel detected the `LEFT JOIN` via `EXPLAIN` plan walking and the catalog reports `name` as nullable.
- The SQL is stored as a module-level constant and passed to asyncpg directly.

## FastAPI integration example

The [`example/`](example/) directory contains a complete, runnable FastAPI application demonstrating the pysquirrel workflow.

### Steps

```bash
# 1. Install pysquirrel
pip install pysquirrel
...
pysquirrel provides two subcommands via [Typer](https://typer.tiangolo.com/):
...
pysquirrel resolves the database connection using the following precedence:
...
pysquirrel maps 40+ built-in Postgres types to Python types. The most common mappings:
...
[MIT](https://github.com/tsegaw/pysquirrel/blob/main/LICENSE)
